...
function CloudConnectionList({ onDelete, onUpdate, onError, rerender, user }) {
    ...
    const featchData = () => {
        ...
        axios.get(`${ip}cloudConnectionDB?page=${page}&size=${parseInt(rows)}&sortField=${sortField}&sortOrder=${parseInt(sortOrder)}&filters=${JSON.stringify(filters)}`)
            .then(({ data }) => {
                setCloudConnections(data.rows);
            }).catch((e) => {
                onError({
                    severity: 'error', summary: 'Fetch CloudConnection',
                    detail: 'Failed to fetch cloud connection from DB', sticky: true
                });
            });
    }
...
export default function CloudConnectionsDB({ onError }) {
    ...
    const deleteRecord = () => {
        axios.delete(`${ip}cloudConnectionDB/${cloudConnection.cloudConnectionId}`).then(() => {
            setShowDeletePopup(false);
            setRerender(Math.random());
            handleError({
                severity: 'success', summary: 'CloudConnection',
                detail: `Successfully deleted CloudConnection in DB`, sticky: true
            });
        }).catch((_) => {
            setShowDeletePopup(false);
            handleError({
                severity: 'error', summary: 'CloudConnection',
                detail: `Error deleting CloudConnection in DB`, sticky: true
            });
        });
    }
    ...
    const createCloudConnectionInfo = (data) => {
        axios.post(`${ip}cloudConnectionDB`, data).then((res) => {
            setShowCreateOrUpdatePopup(false);
            setRerender(Math.random());
            handleError({
                severity: 'success', summary: 'CloudConnection',
                detail: `Successfully created cloudConnection in DB`, sticky: true
            });
        }).catch(({ response: { data } }) => {
            handleError({
                severity: 'error', summary: 'CloudConnection',
    ...
    const updateCloudConnectionInfo = (id, data) => {
        axios.put(`${ip}cloudConnectionDB/${id}`, data).then((res) => {
            setShowCreateOrUpdatePopup(false);
            setRerender(Math.random());
            handleError({
                severity: 'success', summary: 'CloudConnection',
                detail: `Successfully updated CloudConnection in DB`, sticky: true
            });
        }).catch(({ response: { data } }) => {
            handleError({
                severity: 'error', summary: 'CloudConnection',
    ...
    const createCloudConnectionForGcp = (data) => {
        ...
        axios.post(`${ip}cloudConnectionDB/gcp/upload`, data.formData, config).then(() => {
            setShowCreateOrUpdatePopup(false);
            setRerender(Math.random());
            handleError({
                severity: 'success', summary: 'CloudConnection',
    ...
    const updateCloudConnectionForGcp = (id, data) => {
        ...
        data.formData.append('cloudId', data.cloudId);
        data.formData.append('userId', data.userId);
        axios.put(`${ip}cloudConnectionDB/gcp/upload/${id}`, data.formData, config).then(() => {
            setShowCreateOrUpdatePopup(false);
            setRerender(Math.random());
            handleError({
                severity: 'success', summary: 'CloudConnection',
            ...
            <Container>
                <Row>
                    <Col sm={12} style={{ textAlign: 'center' }}>
                        <h3 style={{ marginBottom: 0 }}>Cloud Connections - Database</h3>
                    </Col>
